...
try {
    & $dotnet restore .\src\FotoLibraryCleaner.App\FotoLibraryCleaner.App.csproj --configfile .\NuGet.Config
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }

    & $dotnet run --project .\src\FotoLibraryCleaner.App\FotoLibraryCleaner.App.csproj --no-restore
    exit $LASTEXITCODE
}
finally {
    Pop-Location
